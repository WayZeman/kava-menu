...
export async function checkVisitNoticeCooldown(deviceId, cooldownMinutes = 5) {
  const sql = getSql();
  ...
  const cooldownMs = Math.max(1, Number(cooldownMinutes) || 5) * 60 * 1000;
  if (rows[0]?.last_notified_at) {
    ...
    if (Number.isFinite(lastAt) && Date.now() - lastAt < cooldownMs) {
      return {
        shouldNotify: false,
        reason: 'cooldown',
        retryAfterSec: Math.ceil((cooldownMs - (Date.now() - lastAt)) / 1000),
      };
    }
  }
