...
let deviceLabelsTableReady = false;

async function ensureDeviceLabelsTable(sql) {
  if (deviceLabelsTableReady) return;
  ...
  deviceLabelsTableReady = true;
}

export async function getDeviceLabel(deviceId) {
  const sql = getSql();
  if (!sql) return null;
  ...
  await ensureDeviceLabelsTable(sql);
...
export async function setDeviceLabel(deviceId, displayName) {
  const sql = getSql();
  if (!sql) return null;

  const id = String(deviceId || '').trim();
  const name = String(displayName || '').trim();
  if (!id || id.length > 120 || !name || name.length > 60) return null;

  await ensureDeviceLabelsTable(sql);

  await sql`
    INSERT INTO device_labels (device_id, display_name, updated_at)
    VALUES (${id}, ${name}, NOW())
    ON CONFLICT (device_id) DO UPDATE SET
      display_name = ${name},
      updated_at = NOW()
  ...
  return { deviceId: id, displayName: name };
}

export async function resolveDeviceId(input) {
  const token = String(input || '').trim();
  if (!token || token.length > 120) return null;

  const sql = getSql();
  if (!sql) return token.length >= 12 ? token : null;

  await ensureDeviceLabelsTable(sql);
  await ensureVisitNoticeTable(sql);

  const exactLabel = await sql`
    SELECT device_id FROM device_labels WHERE device_id = ${token} LIMIT 1
  `;
  if (exactLabel[0]?.device_id) return exactLabel[0].device_id;

  const exactVisit = await sql`
    SELECT device_id FROM site_visit_notices WHERE device_id = ${token} LIMIT 1
  `;
  if (exactVisit[0]?.device_id) return exactVisit[0].device_id;
  ...
  const matches = await sql`
    SELECT device_id
    FROM site_visit_notices
    WHERE device_id LIKE ${`${token}%`}
    ORDER BY last_notified_at DESC
    LIMIT 3
  `;

  if (matches.length === 1) return matches[0].device_id;

  const labelMatches = await sql`
    SELECT device_id
    FROM device_labels
    WHERE device_id LIKE ${`${token}%`}
  ...
  if (labelMatches.length === 1) return labelMatches[0].device_id;

  if (token.startsWith('device-') && token.length >= 12) return token;
